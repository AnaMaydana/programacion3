...
int main() {
    ...
    if (p_entero != nullptr) {//Buena práctica: verificar si new tuve éxito (aunque suele Lanzar excepción)
        *p_entero = 123; // Asigna un valor a Le memoria recién reservada
        std::cout << "Entero dinamico creado. Valor:"<< *p_entero
                  <<"en direccion:" << p_entero << std::endl;
        
        delete p_entero;      // Libera la memoria
        p_entero = nullptr;   //¡Buena práctical! Evita puntero colgante.
   ...
   if (p_arreglo_doubles != nullptr) {
    ...
    // Intentar usar un puntero nulo (solo para denastrar, usualmente causa error o comportamiento indefinido)
    // if (p_entero == nullptr) {
    // std::cout << "\np entero es ahora nullptr." << std::endl;
    //    //*p entero = 789; // ¡Esto causaría un error de segmentación! (Descomentar con precaución)
    //}
